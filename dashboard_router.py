...
# ==============================================================================
# PHASE 24 UPDATE:
# FIX: Adjusted start_server_sequence to target state.server_path instead
#      of os.getcwd() so adopted servers launch correctly.
# ==============================================================================
...
class DashboardRouter(QObject):
    ...
    def start_server_sequence(self):
        ...
        # FIXED: Pull the actual working directory from AppState, not the .exe location
        working_directory = getattr(state, "server_path", "")
        
        if not working_directory or not os.path.exists(working_directory):
            self.append_system_log("!! CRITICAL: No active server path configured.")
            self.append_system_log("!! HINT: Adopt a folder in the Configuration Tab first.")
            return
        ...
        self.append_system_log(f"PATH: {working_directory}")
    ...
    def open_file_explorer(self, target):
        """
        Opens Windows Explorer to a specific directory based on the active server.
        """
        working_directory = getattr(state, "server_path", os.getcwd())
        path_to_open = working_directory
        
        if target == "logs":
            path_to_open = os.path.join(working_directory, "7DaysToDieServer_Data")
        elif target == "saves":
            # 7D2D defaults to APPDATA for saves, unless overridden by XML
    ...
    def update_status_led(self, status):
        ...
        """
        if not hasattr(self.ui, 'lbl_system_status'): return
        label_widget = self.ui.lbl_system_status
        
        if status == "ONLINE":
            label_widget.setText("SERVER ONLINE")
            label_widget.setStyleSheet("color: #2ecc71; font-size: 10px; font-weight: bold;") 
        elif status == "BOOTING":
            label_widget.setText("BOOTING UP...")
            label_widget.setStyleSheet("color: #f1c40f; font-size: 10px; font-weight: bold;") 
        else:
            label_widget.setText("SERVER OFFLINE")
            label_widget.setStyleSheet("color: #e74c3c; font-size: 10px; font-weight: bold;") 
